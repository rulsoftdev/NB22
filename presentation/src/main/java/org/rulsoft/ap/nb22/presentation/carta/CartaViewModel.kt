...
import org.rulsoft.ap.nb22.presentation.carta.composable.calculaBase
import org.rulsoft.ap.nb22.presentation.carta.composable.getEtiqueta
import org.rulsoft.ap.nb22.presentation.carta.composable.sumaDigitos
...
val PILARES_KEYS = listOf("pp", "pd", "ne", "ci", "ce", "cp", "cg", "ps", "pa", "be", "de", "pf", "rne", "rci", "rce", "rcp", "rup", "rua")

@KoinViewModel
class CartaViewModel (
    private val logService: CrashlyticsLogger,
    ...
    private fun actualizarPilares(
        pilaresExistente: MutableMap<String, Pilar>,
        carta: Carta
    ): Map<String, Pilar> {
        val pilaresActualizados = pilaresExistente.toMutableMap()

        // val mapLogs = mutableMapOf<String, String>()
        // mapLogs["Carta"] = carta.toString()
        // Loguear el evento principal con los datos de la carta.

        pilaresExistente.forEach { key, pilar ->
            try {
                // Loguear detalles de cada pilar antes de la actualización.
                // logService.logEvent("ActualizarPilar", mapOf("PilarKey" to key, "PilarActual" to pilar.toString()))
                // mapLogs[key] = pilar.valor.toString()
                // Obtener el valor de la propiedad asociado al pilar.
                val valor = carta.obtenerValorPropiedad(key) as Int

                // Loguear el valor obtenido.
                // logService.logMessage("ActualizarPilar -> Key: $key, Valor: $valor")

                // Crear una copia del pilar actualizado.
                val pilarActualizado = pilar.copy(
                    etiqueta = getEtiqueta(valor),
                    valor = calculaBase(valor)
                )

                // Actualizar repeticiones según el valor.
                actualizaRepeticiones(valor)

                // Guardar el pilar actualizado en el mapa.
                pilaresActualizados[key] = pilarActualizado

            } catch (e: Exception) {
                // Loguear cualquier excepción ocurrida durante el procesamiento de un pilar.
                logService.logException(RuntimeException("Error al actualizar el pilar con Key: $key", e))
            }
        }
